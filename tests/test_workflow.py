...
EVOLUTION_RESPONSE = json.dumps({
    ...
    "specific_refinements": [
        {"aspect": "clarity", "change": "Reworded", "justification": "Clearer"},
    ],
})
...
SUPERVISOR_RESPONSE = json.dumps({
    ...
    "workflow_plan": {
        ...
        "review_phase": {"critical_criteria": ["soundness"], "review_depth": "standard"},
        "ranking_phase": {"ranking_approach": "composite", "selection_criteria": ["score"]},
        "evolution_phase": {"refinement_priorities": ["clarity"], "iteration_strategy": "top-k"},
    },
})
...
def _build_framework():
    ...
    with patch("ai_coscientist.main.Agent") as MockAgentCls:
        ...
        agent_map["HypothesisGenerator"].run.return_value = GENERATION_RESPONSE
        agent_map["HypothesisReflector"].run.return_value = REFLECTION_RESPONSE
        agent_map["HypothesisRanker"].run.return_value = RANKING_RESPONSE
        agent_map["TournamentJudge"].run.return_value = TOURNAMENT_RESPONSE
        agent_map["HypothesisEvolver"].run.return_value = EVOLUTION_RESPONSE
        agent_map["MetaReviewer"].run.return_value = META_REVIEW_RESPONSE
        agent_map["ProximityAnalyzer"].run.return_value = PROXIMITY_RESPONSE
...
def test_workflow_returns_dict_with_expected_keys():
    ...
    for key in ("top_ranked_hypotheses", "meta_review_insights",
                "execution_metrics", "total_workflow_time"):
...
def test_workflow_execution_metrics_structure():
    ...
    for key in ("total_time", "hypothesis_count", "reviews_count",
                "tournaments_count", "evolutions_count",
